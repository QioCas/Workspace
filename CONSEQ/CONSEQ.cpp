#define console(...) void() // 2022/11/27-08:20:27
#define READ
#define CHECK
...
int digit(BigInt a, int res = 0) {
    while(a.isZero() == false) a /= 10, res += 1;
    return res;
}
BigInt after(BigInt m) {
    int dig = digit(m); BigInt newnumber = 1;
    for(int i = 1; i <= dig; ++i) newnumber *= 10;
    return newnumber;
}
signed main() {
    ...
    cin >> w >> m >> k;
    BigInt bef_m = m;
    for(BigInt v = after(m); BigInt(digit(m)) * (v - m) * k <= w;) {
        BigInt sub = BigInt(digit(m)) * (v - m) * k;
        w -= sub;
        m = v; v = after(m);
    }
    // console(m, w);
    BigInt left = m, right = after(m);
    BigInt dig = BigInt(digit(m));
    while(left < right) {
        BigInt mid = (left + right) / 2;
        // console(left, right, mid);
        // ans = (v - m + 1) * k >  m (min v)
        if(dig * (mid - m + 1) * k > w) right = mid; 
        else left = mid + 1;
    }
    cout << (left - bef_m);
}